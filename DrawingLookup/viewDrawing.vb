...
FileExists_Error:
    ...
    'Check to see if the file on record is a single file name meant to be viewed from the Push Folder
    'OR if file on record is a full path
    ...
    Public Function getId(ByVal partId As String)
        id = partId
        selectCellClickPartQuery = "Select partID, drawingType, drawingRevision, partName, drawingType, drawingRevision, drawingDate, fileName from partnames where partId=" + id + ""
    ...
    'Function that queries text when more than 2 characters are inputed into search Text Box
    ...
    Private Sub OpenFileDialog1_FileOk(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles OpenFileDialog1.FileOk
        fileTxtVD.Text = OpenFileDialog1.FileName
    End Sub

    Private Sub disableChk_CheckedChanged(sender As Object, e As EventArgs) Handles disableChk.CheckedChanged
        If (disableChk.Checked) Then

            DateTimePicker1.Enabled = False
            createBtn.Enabled = False
            deleteBtn.Enabled = False
            updateBtn.Enabled = False
            partTxt.Enabled = False
            revCbx.Enabled = False
            authorCbx.Enabled = False
            typeCbx.Enabled = False
            browseBtn.Enabled = False
            fileTxtVD.Enabled = False
        Else
            DateTimePicker1.Enabled = True
            createBtn.Enabled = True
            deleteBtn.Enabled = True
            updateBtn.Enabled = True
            partTxt.Enabled = True
            revCbx.Enabled = True
            authorCbx.Enabled = True
            typeCbx.Enabled = True
            browseBtn.Enabled = True
            fileTxtVD.Enabled = True
    ...
    'Function(fileAssign) that determines how files and filepaths will be retrieved when a record is selected
    ...
    Public Function fileUpdate(ByVal drawingLink As String)
        ...
        If isDir = False And isDir2 = False Or drawingLink = "" Then
            If FileExists(My.Settings.drawingFolder + "\" + drawingLink) = True Or FileExists(drawingLink) = True Then
                Dim newDrawingType As String
                Dim drawingTypeAcronym As String
                newDrawingType = typeCbx.Text
                Select Case newDrawingType
                    Case "Component"
                        drawingTypeAcronym = "PD"
                    Case "Assembly"
                        drawingTypeAcronym = "AD"
                    Case "Machine"
                        drawingTypeAcronym = "MD"
                    Case "Finish"
                        drawingTypeAcronym = "FD"
                    Case "Customer"
                        drawingTypeAcronym = "CD"
                    Case "Vendor"
                        drawingTypeAcronym = "VD"
                    Case "Roller"
                        drawingTypeAcronym = "RD"
                    Case "Cage"
                        drawingTypeAcronym = "XD"
                    Case Else
                        drawingTypeAcronym = ""
                End Select
                ...
                newFile = partTxt.Text + "-" + drawingTypeAcronym + "-" + newDate + "-REV" + revCbx.Text + ".pdf"
                If Me.DataGridView1(11, (DataGridView1.CurrentRow.Index)).Value = drawingLink Then 'if record file is the same as file
                    ...
                    End If
                Else
                    If FileExists(My.Settings.drawingFolder + "\" + drawingLink) = True Then
                        If isDir = False Then
                            If IsRootFile(drawingLink) = False Then 'if record holds a filename and not a file path
                                Try
                                    newFile = fileTxtVD.Text
                                    Return newFile
                                ...
                                Catch ex As IOException
                                    MsgBox("The file that was entered already exists in the Drawing Folder")
                                    newFile = Nothing
                                    Return newFile
                                End Try
                            End If
                    ...
                    Else
                        ...
                        MsgBox("The file entered does not exist in the Drawing Folder")
                        newFile = Nothing
                        Return newFile
                    End If
                End If
            Else
                If Me.DataGridView1(11, (DataGridView1.CurrentRow.Index)).Value.ToString <> fileTxtVD.Text Then
                ...
                If MessageBox.Show("Are you sure you want to update this record to: " & vbCrLf &
                  ...
                  "From: " & vbCrLf & vbCrLf &
                   ...
                   "Part Type: " & Me.DataGridView1(3, id1).Value & vbCrLf &
                    ...
                    "Date: " & Me.DataGridView1(4, id1).Value, "Update A Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = MsgBoxResult.Yes Then

                    If Me.DataGridView1(11, id1).Value IsNot Nothing Then
                        ...
                        Else
                            ...
                            If Me.DataGridView1(11, id1).Value = file1 Or Me.DataGridView1(11, id1).Value = file3 Then
                                If match = 1 And Me.DataGridView1(11, id1).Value = newFile Then
                                    'handle query
                                    ...
                                    If newFile = valls Then 'if Drawing LINK text doesn't change
                                        ...
                                        cmd.ExecuteNonQuery()
                                        dbConn.Close()
                                    Else ' If DRAWING LINK TEXT IS DIFFERENT THAN THE ONE ON RECORD
                                        If IsRootFile(newFile) = True Then ' and its a filePath --> (C://)
                                        ...
                                        End If
                                    End If
                                ElseIf match = 1 And Me.DataGridView1(11, id1).Value <> newFile Then
                                    Dim fileinput1 = newFile
                                    Dim inputMatch = inputLinkMatch(fileinput1)
                                    If inputMatch = 0 Then
                                        dbConn.Open() 'OPEN------------
                                        cmd.ExecuteNonQuery()
                                        dbConn.Close()
                                    Else
    ...
    Public Function inputLinkMatch(ByVal matchQuery As String) As Integer
        Dim loginQuery = "SELECT Count(*) AS Expr1 FROM partnames WHERE fileName = @fileName"
        Dim loginCommand = New OleDbCommand(loginQuery, dbConn)
        If matchQuery Is Nothing Then
            matchQuery = "%%%%\w(?<!\w)%%%"
        End If
        loginCommand.Parameters.Add("@fileName", OleDbType.VarChar, 255).Value = matchQuery
        dbConn.Open() 'OPEN------------
    ...
    Public Function drawingLinkMatch(ByVal matchQuery As String, ByVal matchQuery2 As String) As Integer
        Dim loginQuery = "SELECT Count(*) AS Expr1 FROM partnames WHERE fileName = @drawingLink or fileName = @drawingLink2"
        Dim loginCommand = New OleDbCommand(loginQuery, dbConn)
    ...
    Private Sub createBtn_Click(sender As Object, e As EventArgs) Handles createBtn.Click
        ...
        If My.Computer.FileSystem.FileExists(fileTxtVD.Text) Then
            ...
            Else
                ...
                Dim sql = "INSERT INTO partnames( partName, drawingType,  drawingAuthor, fileName, " & _
                ...
                If MessageBox.Show("Are you sure you want to create this record: " & vbCrLf &
                    ...
                    'Filling parameters with user input
                    Using cmd = New OleDb.OleDbCommand(sql, dbConn)
                        ...
                        End If
                        cmd.Parameters.AddWithValue("@p4", Path.GetFileName(fileTxtVD.Text))
                        ...
                        If fileTxtVD.Text = "" Then
                            MsgBox("A file path must be inserted before creating the record.", MsgBoxStyle.Exclamation)
                        Else
                            Dim fileInput1 = Path.GetFileName(fileTxtVD.Text)
                            ...
                            Else
                                ...
                                For Each row As DataGridViewRow In DataGridView1.Rows
                                    If row.Cells(0).Value > MaxVal Then
                                        ...
                                        DataGridView1.Rows(row.Index).Selected = True
                                        My.Computer.FileSystem.CopyFile(fileTxtVD.Text, My.Settings.pushFolder + "\" + DataGridView1.Rows(row.Index).Cells(11).Value.ToString())
                                        fileAssign(DataGridView1.Rows(row.Index).Cells(11).Value.ToString())
                                    End If
                                Next
                                'Copy file to drawing folder
            ...
            End If
        Else
            MsgBox("File cannot be found. Make sure the filepath exists.", , "File Not Found")
            Exit Sub
